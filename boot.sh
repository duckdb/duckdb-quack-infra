...
if [ -f /home/ubuntu/rpc-token ]; then
    MY_TOKEN=$(cat /home/ubuntu/rpc-token)
else
...
SET VARIABLE my_token = '${MY_TOKEN}';
SET rpc_default_token = '${MY_TOKEN}';
SQL
...
if [ "${USER_SQL}" = "200" ]; then
    ...
    echo "CALL rpc_start('quack:0.0.0.0:1294', allow_other_hostname=true);" > /tmp/user_init.sql
fi
...
echo "Ready at https://${MY_HOSTNAME}/rpc"
echo "Token: ${MY_TOKEN}"
echo "${MY_TOKEN}" > /home/ubuntu/rpc-token
